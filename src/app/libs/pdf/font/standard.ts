...
export class StandardFont extends PDFFont {
  font: AFMFont;
  bbox: number[];
  constructor(document: PDFDocument, name: string, id: string) {
    super();
  ...
  override encode(text: string, features?: string[]) {
    ...
    const positions: FontGlyphPosition[] = [];
    ...
    const result: [string[], FontGlyphPosition[]] = [encoded, positions];
    return result;
  }

  override widthOfString(str: string, size: number) {
    const glyphs = this.font.glyphsForString(`${str}`);
    const advances = this.font.advancesForGlyphs(glyphs);
