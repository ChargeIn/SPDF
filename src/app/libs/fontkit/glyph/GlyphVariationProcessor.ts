...
export default class GlyphVariationProcessor {
  ...
  normalizeCoords(coords) {
    ...
    const normalized = [];
    for (i = 0; i < this.font.fvar.axis.length; i++) {
    ...
    if (this.font.avar) {
      for (i = 0; i < this.font.avar.segment.length; i++) {
  ...
  transformPoints(gid, glyphPoints) {
    ...
    for (let i = 0; i < tupleCount; i++) {
      ...
      if (points.length === 0) {
        ...
        for (let j = 0; j < glyphPoints.length; j++) {
          const deltaX = outPoints[j].x - origPoints[j].x;
          const deltaY = outPoints[j].y - origPoints[j].y;

          glyphPoints[j].x += deltaX;
          glyphPoints[j].y += deltaY;
        }
      }
  ...
  getAdvanceAdjustment(gid, table) {
    ...
    if (table.advanceWidthMapping) {
      ...
      const entryFormat = table.advanceWidthMapping.entryFormat;
