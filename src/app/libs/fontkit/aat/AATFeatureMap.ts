...
const feature = (name, selector) => [
  featuresMap[name].code,
  featuresMap[name][selector],
];
...
for (let i = 1; i <= 99; i++) {
  OTMapping[`cv${`00${i}`.slice(-2)}`] = [
    featuresMap.characterAlternatives.code,
    i,
  ];
}
...
function mapFeatureStrings(f) {
  ...
  if (isNaN(type)) {
    typeCode = featuresMap[type] && featuresMap[type].code;
  } else {
    typeCode = type;
  }

  if (isNaN(setting)) {
    settingCode = featuresMap[type] && featuresMap[type][setting];
  } else {
    settingCode = setting;
  }
...
export function mapAATToOT(
  features: string[] | { [key: string]: { [key: string]: any } }
) {
  const res = {};
  if (Array.isArray(features)) {
    ...
    for (const type in features) {
      ...
      for (const setting in feat) {
        let r;
        const f = mapFeatureStrings([type, setting]);
        if (feat[setting] && (r = AATMapping[f[0]] && AATMapping[f[0]][f[1]])) {
          res[r] = true;
        }
      }
    }
